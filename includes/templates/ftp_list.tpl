...
{elseif {$smarty.request.routine|default:''} == 'confirm_clear_ssh'}
	{assign var='msg' value="Are you sure you want to clear the SSH (SFTP) list?"}
...
{/if}
	...
	{if $smarty.request.filter_include_ftp|default:'' != '' || $smarty.request.filter_include_ssh|default:'' != '' || $smarty.request.filter_domains_include|default:'' != '' || $smarty.request.filter_domains_exclude|default:'' != '' || isset($country_value_set) || $smarty.request.filter_date_from|default:'' != '' || $smarty.request.filter_date_to|default:'' != '' || $smarty.request.filter_text|default:'' != ''}
		<h4>Attention! Filter activated. <a href="{$smarty.server.SCRIPT_NAME}?action=ftp">Cancel</a>.</h4><br />
	{/if}

	<a class="download_nav" href="{$smarty.server.SCRIPT_NAME}?action=ftp&amp;routine=download_ftp">Download FTP list</a> (<b>{$total_ftp_items_count}</b> {$total_ftp_items_count|items})<br />
	<a class="download_nav" href="{$smarty.server.SCRIPT_NAME}?action=ftp&amp;routine=download_ssh">Download SSH (SFTP) list</a> (<b>{$total_ssh_items_count}</b> {$total_ssh_items_count|items})<br />
	{if $priv_can_delete}
		<a class="delete_nav" href="{$smarty.server.SCRIPT_NAME}?action=ftp&amp;routine=confirm_clear_ftp">Clear FTP list</a><br />
		<a class="delete_nav" href="{$smarty.server.SCRIPT_NAME}?action=ftp&amp;routine=confirm_clear_ssh">Clear SSH (SFTP) list</a><br />
	{/if}
	...
	{if isset($filtered_items_count)}
		{if $filtered_items_count > 0}
			(найдено <b>{$filtered_items_count}</b> {$filtered_items_count|items}, <a href="#" onclick="javascript:$('#routine').val('filter_download'); $('#filter_frm').submit(); $('#routine').val(''); return false;"><b>скачать</b></a>)
		{else}
			(No results)
		{/if}
	{/if}
	...
	<div id="filter_frm_block" style="display:none;width:500px">
		...
		<div id="tabs">
			...
			<div id="tabs-1">
				...
				<p style="margin-left:10px;">Include domains: <br /><input style="width:250px;" name="filter_domains_include" value="{$smarty.request.filter_domains_include|default:''}" /> <br /><small>(You can specify multiple comma-separated)</small></p><br />
				...
				<p style="margin-left:10px;">Search text: <br /><input style="width:250px;" name="filter_text" value="{$smarty.request.filter_text|default:''}" /> <br /><small>(substring search, works very fast)</small></p><br />
					...
					{if $smarty.request.filter_trim_dirs|default:'' == '1'}
						checked="checked"
					{/if}
					/><label for="filter_trim_dirs"> Exclude directories from the FTP/SFTP lines</label><br /><br />
	...
	{if count($ftp_list)}
		{if isset($filtered_items_list)}
			<h4>Example Filtered FTP accounts</h4>
			<table id="table_ftp" width="800px" cellspacing="0" summary="FTP filter examples">
		{else}
			<h4>Latest FTP</h4>
			<table id="table_ftp" width="800px" cellspacing="0" summary="Latest FTP additions">
		{/if}

		<tr>
			<th width="53%">URL</th>
			<th width="27%">FTP client</th>
		...
		{foreach from=$ftp_list item=ftp_item}
			...
			{/if}
			{$ftp_item.url|truncate:60:'...':true:false}</td>
